...
npm run changelog     # regenerate CHANGELOG.md from git history (scripts/gen-changelog.mjs)
npm run test          # vitest run (unit + component tests)
npm run test:watch    # vitest in watch mode
npm run test:e2e      # playwright (chromium) — builds + previews at :4173
npm run test:all      # npm run test && npm run test:e2e
```

No linter is configured. Test stack:

- **Vitest** + **jsdom** — unit tests for `src/lib/utils/` and component tests for Svelte 5 components via `@testing-library/svelte`. Test files live next to source in `__tests__/` folders; component test files use the `.svelte.test.ts` suffix. Setup file: [`tests/setup.ts`](tests/setup.ts). `vite.config.ts` sets `resolve.conditions: ['browser']` under `VITEST` so `@testing-library/svelte` resolves Svelte's client entry.
- **Playwright** (chromium-only) — e2e in `tests/e2e/`. The config auto-runs `npm run build && npm run preview -- --port 4173` via `webServer`, so tests always run against the production build, not `vite dev`.
- **Data-shape sanity test** — `src/lib/utils/__tests__/data.test.ts` loads `static/data/journeys.json` and validates counts (57/114/15/3), phase + jurisdiction enums, no duplicate ids, no orphan step refs, no dangling dependencies, no self-edges, and acyclicity of the non-parallel dependency graph. This is the authoring gate — if you add or edit a journey, this test will catch structural breakage before `topoSort` silently does the wrong thing.

`npm run check` (svelte-check) + `npm run test:all` is the full pre-commit gate. CI runs the same checks in parallel jobs via [`.github/workflows/ci.yml`](.github/workflows/ci.yml) on PRs and pushes to master.
...
│   │   │                           SourcesPanel, TopNavbar, JourneyRow, ScreenSizeNotice
...
├── CHANGELOG.md                 ← generated by `npm run changelog` from git history
